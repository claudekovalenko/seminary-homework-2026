...
const bareText = (text) => String(text || '').replaceAll(EM, '').replaceAll(STRONG, '');

/** Out of the app the markers mean nothing, so they leave as markdown. */
const toMarkdown = (text) => String(text || '').replaceAll(STRONG, '**').replaceAll(EM, '*');
...
return { EM, STRONG, bareText, toMarkdown, emphasisRuns };
})();
const __mod_store = (() => {
...
const highlightsFor = (id) => state.highlights[id] || [];

const highlightKey = (h) => `${h.page}:${h.para}:${h.start}`;
...
function isHighlighted(id, span) {
